<?php $__env->startSection('title', 'Suivi de Presence'); ?>
<?php $__env->startSection('page-name', 'attendance'); ?>
...
<div class="min-h-screen bg-pattern">
    ...
    <div class="sticky top-0 z-20 bg-base-100 border-b border-base-300">
        <div class="max-w-lg mx-auto px-4">
            <div class="flex items-center justify-between gap-3 py-3">
                <form method="POST" action="<?php echo e(route('attendance.navigate')); ?>" class="inline">
                    <?php echo csrf_field(); ?>
                    <input type="hidden" name="current_date" value="<?php echo e($date); ?>">
                ...
                <button onclick="toggleCalendar()" class="flex-1 text-center py-2 px-4 bg-blue-600 text-white rounded-lg font-semibold text-sm flex items-center justify-center gap-2">
                    ...
                    <span><?php echo e(\Carbon\Carbon::createFromFormat('Y-m-d', $date)->locale('fr')->isoFormat('dddd D MMMM')); ?></span>
                </button>
    ...
    <!-- Compteurs -->
    <div class="sticky top-[60px] z-10 bg-base-100 border-b border-base-300 <?php echo e($isCompleted ? 'opacity-50' : ''); ?>">
        <div class="max-w-lg mx-auto px-4 py-3">
            <div class="grid grid-cols-3 gap-3">
                <div class="bg-base-200 rounded-lg p-3 text-center">
                    <p class="text-xs text-base-content/60 uppercase font-medium">Total</p>
                    <p id="counter-total" class="text-2xl font-bold"><?php echo e($total); ?></p>
                </div>
                <div class="bg-success/10 rounded-lg p-3 text-center">
                    <p class="text-xs text-success uppercase font-medium">Presents</p>
                    <p id="counter-present" class="text-2xl font-bold text-success"><?php echo e($present); ?></p>
                </div>
                <div class="bg-error/10 rounded-lg p-3 text-center">
                    <p class="text-xs text-error uppercase font-medium">Absents</p>
                    <p id="counter-absent" class="text-2xl font-bold text-error"><?php echo e($absent); ?></p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="max-w-lg mx-auto px-4 py-4 pb-6">
        ...
        <?php endif; ?>
        <div class="space-y-6 <?php echo e($isCompleted ? 'opacity-50 pointer-events-none' : ''); ?>">
            <?php $__empty_1 = true; $__currentLoopData = $employeesByRole; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $roleName => $roleEmployees): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <div>
                    <!-- Titre du metier -->
                    <div class="flex items-center gap-2 mb-3">
                        <div class="w-1 h-5 bg-blue-600 rounded-full"></div>
                        <h3 class="text-sm font-bold uppercase tracking-wide"><?php echo e($roleName); ?></h3>
                        <span class="badge badge-ghost badge-sm"><?php echo e(count($roleEmployees)); ?></span>
                    </div>
                    ...
                    <div class="space-y-2">
                        <?php $__currentLoopData = $roleEmployees; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $employee): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            ...
                            <div
                                ...
                                data-is-present="<?php echo e($isPresent ? '1' : '0'); ?>"
                                onclick="toggleAttendance(<?php echo e($employee->id); ?>)"
                                class="employee-card card bg-base-100 <?php echo e($isCompleted ? 'locked' : 'cursor-pointer'); ?> <?php echo e($isPresent ? 'border-2 border-success bg-success/5' : 'border border-base-300 hover:border-blue-500/30'); ?>"
                            >
                                <div class="card-body p-4 flex-row items-center justify-between">
                                    <div class="flex items-center gap-3">
                                        <div class="w-10 h-10 rounded-xl flex items-center justify-center transition-all <?php echo e($isPresent ? 'bg-success text-success-content' : 'bg-base-200'); ?>">
                                            <?php if($isPresent): ?>
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <span class="font-medium <?php echo e($isPresent ? 'text-success' : ''); ?>">
    ...
    <div class="max-w-lg mx-auto px-4 pt-4 pb-24">
        <?php if($isCompleted): ?>
            <form method="POST" action="<?php echo e(route('attendance.reopen')); ?>">
                <?php echo csrf_field(); ?>
        ...
        <?php else: ?>
            <form method="POST" action="<?php echo e(route('attendance.complete')); ?>" id="complete-form">
                <?php echo csrf_field(); ?>
                <input type="hidden" name="date" value="<?php echo e($date); ?>">
        ...
        <div class="bg-base-100 rounded-2xl shadow-xl w-full max-w-sm transform scale-95 opacity-0 transition-all duration-300" id="completion-modal-content">
            <div class="p-6 text-center">
                ...
                <p class="text-base-content/60 mb-6">
                    L'appel du <span class="font-semibold"><?php echo e(\Carbon\Carbon::createFromFormat('Y-m-d', $date)->locale('fr')->isoFormat('dddd D MMMM')); ?></span> sera marqué comme terminé.
                    <br><br>
                    <span class="text-sm">
                        <strong class="text-success" id="modal-present-count"><?php echo e($present); ?></strong> présent(s) •
                        <strong class="text-error" id="modal-absent-count"><?php echo e($absent); ?></strong> absent(s)
                    </span>
                </p>
...
<script>
    ...
    let calendarCurrentDate = new Date('<?php echo e($date); ?>');
    ...
    function renderCalendar() {
        ...
        const selectedDate = new Date('<?php echo e($date); ?>');
    ...
    function selectDate(year, month, day) {
        ...
        form.method = 'POST';
        form.action = '<?php echo e(route("attendance.navigate")); ?>';
    ...
    async function toggleAttendance(employeeId) {
        ...
        try {
            const response = await fetch('<?php echo e(route("attendance.toggle")); ?>', {
                method: 'POST',
                headers: {
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Shadow\benka\resources\views/attendance.blade.php ENDPATH**/ ?>